function [reactions] = getRXN_Velocity_INS1E(y,params)
%% Initial Conditions
Glu_in         = y(1);      % Glucose
...
%% The Vq values (setting the Vr values to constants)
% Vq_tpi              = 3.4/28; 
% Vq_gapdh            = 0.331/0.413;
% Vq_pgk              = 8.7/2.5;
% Vq_pgam             = 0.94/0.36;
% Vq_eno              = 0.34/0.38;
% Vq_ak               = (200/6)/13.34;
% Vq_rpi              = 426.6666667/181.67;
% Vq_rpe              = 773.6666667/1111.17;
% Vq_tk2              = ((3.64e+19)/60)/((1.23e+18)/60);
% Vq_ta               = ((1.36e+20)/60)/((7.53e+19)/60);
% Vq_gpt              = (66.4/60)/(1.6725e-06);
...
%LDH- Fridyland equation
% KmLD = 47.5;
% Klnc = 1;
% LDH_reaction = Vf_ldh*(Pyr / (KmLD + Pyr))*((NADH/NAD)/(Klnc + (NADH/NAD)));
...
Kicoash_cs          = 0.067;
Kiscoa_cs           = 0.140;
CS_reaction = Vf_cs*(mOAA*mAcCoA - (mCoASH*mCIT/Keq_cs))/...
    ((Kioaa_cs*(1 + (mCIT/Pcit)/Kicit_cs)*Kaccoa_cs) + ...
    (Koaa_cs*(1 + (mCIT/Pcit)/Kicit_cs)*mAcCoA) + ...
    (Kaccoa_cs*(1 + (mATP/Patp)/Kiatp_cs + (mADP/Padp)/Kiadp_cs + (mAMP/Pamp)/Kiamp_cs + mCoASH/Kicoash_cs + mSCoA/Kiscoa_cs)*mOAA) + ...
    mOAA*mAcCoA);
...
%ALDR_reaction = 0;
ALDR_reaction = E_aldr * (k1_aldr*NADPH*Glu_in - (k2_aldr*(Ct_PyP - NADPH)*Sor)/Keq_aldr) ...
